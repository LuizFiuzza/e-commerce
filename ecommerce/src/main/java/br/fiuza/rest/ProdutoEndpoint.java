...
public class ProdutoEndpoint
{
   ...
   {
      TypedQuery<Produto> findByIdQuery = em.createQuery("SELECT DISTINCT p FROM Produto p LEFT JOIN FETCH p.fabricante WHERE p.id = :entityId ORDER BY p.id", Produto.class);
   ...
   {
      TypedQuery<Produto> findAllQuery = em.createQuery("SELECT DISTINCT p FROM Produto p LEFT JOIN FETCH p.fabricante ORDER BY p.id", Produto.class);
